<tr class="product_row" row_id='{{$product->sub_sku}}'>
    <td>
    ...
    <td>
        <input type="text" class="form-control input_number input_quantity_argument" value="{{intval($product->qty_available)}}" readonly name="products[{{$row_index}}][qty_available]">
    </td>
    <td>
        ...
        <input type="text" class="form-control product_quantity input_number input_quantity" value="{{@intval($product->quantity_ordered)}}" name="products[{{$row_index}}][quantity]" @if($product->unit_allow_decimal == 1) data-decimal=1 @else data-rule-abs_digit="true" data-msg-abs_digit="@lang('lang_v1.decimal_value_not_allowed')" data-decimal=0 @endif
        data-rule-required="true" data-msg-required="@lang('validation.custom-messages.this_field_is_required')"

        @if($product->enable_stock) data-rule-min-value="1" data-msg-min-value="
        Giá trị 0 khôngđược phép"
        data-qty_available="{{$product->qty_available}}"
        data-msg_mix_default="Giá trị 0 khôngđược phép"
        @endif >
        {{$product->unit}}
    </td>
    <td>
        <input type="text" name="products[{{$row_index}}][unit_price]" class="form-control product_unit_price input_number" value="{{@number_format($product->last_purchased_price, 0)}}">
    </td>
    <td>
        <input type="text" readonly name="products[{{$row_index}}][price]" class="form-control product_line_total" value="{{@number_format($product->quantity_ordered*$product->last_purchased_price, 0)}}">
    </td>
    @php
    $statusQuantity = intval($product->qty_available) - 1;
    if ($statusQuantity > 0) {
    $status="Thiếu";
    ...
    @endphp
    <td>
        <input type="text" readonly name="products[{{$row_index}}][status]" class="form-control quantity_status" value="{{$status}}">
    </td>
