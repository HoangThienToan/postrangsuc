...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        if (!$is_direct_sale) {
            $total_before_tax = $input['price_total'];
        } else {
            ...
            foreach ($products as $product) {
                ...
                if (!empty($product['transaction_sell_lines_id'])) {
                    ...
                    $line = [
                        ...
                        'price' => isset($product['price']) && !empty($product['price']) ? (float)str_replace(',', '', $product['price']) : 0
                    ];
            ...
            foreach ($buy as $buy) {
                if (!empty($buy['buy_id_edit'])) {
                    ...
                    $linebuy = [
                        ...
                        'price' => isset($buy['price']) && !empty($buy['price']) ? (float)str_replace(',', '', $buy['price']) : 0.0
                    ];
     ...
     * @return object
     */
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type, $electronic_bill = false)
    {
        $il = $invoice_layout;
        ...
        $translatedString = $this->convertNumberToEnWords(abs($total_amount));

        $words  = ucfirst($this->convert_number_to_words($total_amount)) . ' đồng (' . ucfirst($translatedString) . ' Vietnam dong)';
        ...
        // pay men
        if ($output['accountNumber'] && $output['bankCode'] && ($total_amount > 0)) {
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final' || $electronic_bill) {
            ...
            if ($il->show_payments == 1) {
                ...
                $output['payments_primitive'] = $payments;
                if ($electronic_bill) {
        ...
        $output['is_direct_sale'] = $transaction->is_direct_sale;
    ...
    function convertNumberToEnWords($number)
    {
        $ones = array(
            0 => 'zero',
            1 => 'one',
        ...
        if ($number < 20) {
            ...
            $remainder = $number % 100;
            if ($remainder) {
                $words .= ' ' . $this->convertNumberToEnWords($remainder);
            }
        } else {
            $baseUnit = pow(1000, floor(log($number, 1000)));
            $numBaseUnits = floor($number / $baseUnit);
            $remainder = $number % $baseUnit;
            $words .= $this->convertNumberToEnWords($numBaseUnits) . ' ' . $this->getUnitName($baseUnit);
            if ($remainder) {
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        $transaction = Transaction::where('id', $transaction_id)
            ...
            ->with(['sell_lines', 'payment_lines', 'buy_lines'])
            ->first();
        ...
        if (!empty($transaction)) {
            ...
            //Delete electronic bill
            if (!empty($transaction->Key)) {
                $api_elec = new ApiMatBao;
                $api_elec->cancelInvoice($transaction->Key);
            }
        }
    ...
    private function convertLength($str)
    {
        $num = intval($str);
    ...
    public function setBeneficiaryOrganization($acquierID, $consumerID)
    {
        ...
        $beneficiaryOrganizationLength = $this->convertLength(strlen($acquier . $consumer));

        $consumerAccountInformationLength = $this->convertLength($beneficiaryOrganizationLength + 30);
    ...
    public function setAdditionalDataFieldTemplate($content)
    {
        ...
        $additionalDataFieldTemplateLength = $this->convertLength(strlen($content) + 4);
        $this->additionalDataFieldTemplate = "62" . $additionalDataFieldTemplateLength . "08" . $contentLength . $content;
        return $this;
    }

    public function calcCRC($str)
    {
        $crcTable = [
    ...
    public function build()
    {
        ...
        $crc = strtoupper(dechex($this->calcCRC($contentQR)));
